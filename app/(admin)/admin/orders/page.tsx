  ...
  return (
    <div className="grid gap-5">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {statuses.map(([value, label]) => <a key={value} href={`/admin/orders?status=${value}`} className="rounded-[2rem] bg-white p-4 shadow-soft"><b className="text-claret">{label}</b><p className="mt-2 text-sm text-gray-500">{orders.filter((order) => order.orderStatus === value).length} đơn</p></a>)}
      </div>
