...
export function CartSummary() {
  ...
  return (
    <aside className="rounded-[2rem] border border-silver-200 bg-white p-5 shadow-soft">
        ...
        {items.map((item) => (
          <div key={item.variantId} className="flex gap-3">
            ...
            <div className="flex-1 text-sm">
              ...
              <p className="text-gray-500">{item.variantTitle} x {item.quantity}</p>
            </div>
            <b className="text-sm">{formatCurrency(item.unitPrice * item.quantity)}</b>
          </div>
        ))}
      </div>
      <div className="mt-5 grid gap-2 border-t border-silver-200 pt-4 text-sm">
        ...
        <div className="flex justify-between text-lg"><span>Tổng dự kiến</span><b className="text-claret">{formatCurrency(subtotal + shipping)}</b></div>
      </div>
    </aside>
  );
}
